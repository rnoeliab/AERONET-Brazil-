...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 24 10:22:39 2020
...
def modis_all(INPUT_AERONET_MODIS,data,city,resol,listdir,prod):
    ...
    corre_terra = []
    for w in range(3):
        t_subset = terra.dropna(subset=['AOD modis',str(name[w])])
    ...
    corre_aqua = []
    for w in range(3):
        a_subset = aqua.dropna(subset=['AOD modis',str(name[w])])
...
for city in cities:
    ...
    for r,p in zip(resol,product):
        ...
        count = count + 1
data_corre.to_csv(OUTPUT+"tabla_correlacion_MODIS.csv", index=False)
...
####################### correlation calculation  #########################
    ...
    for temp in ['DJF','MAM','JJA','SON']:
        for w in range(3):
            t_subset = terra.loc[terra.loc[:,'temporada'] == str(temp)].dropna(subset=['AOD modis',str(name[w])])
            X = t_subset[str(name[w])]
            Y = t_subset["AOD modis"]
            num_t.append(len(t_subset))
            if (len(t_subset) == 0):
    ...
    for temp in ['DJF','MAM','JJA','SON']:
        for w in range(3):
            a_subset = aqua.loc[aqua.loc[:,'temporada'] == str(temp)].dropna(subset=['AOD modis',str(name[w])])
            X = a_subset[str(name[w])]
            Y = a_subset["AOD modis"]
            num_a.append(len(a_subset))
            if (len(a_subset) == 0):
    ...
    for r,p in zip(resol,product):
        ...
        for n,temp in enumerate(['DJF','MAM','JJA','SON']):
            for w in range(3):
                ...
                suma = suma + 1
            count = count + 1
data_corre.to_csv(OUTPUT+"tabla_correlacion_MODIS_temp.csv", index=False)
...
for city in cities:
    ...
    for n,temp in enumerate(['DJF','MAM','JJA','SON']):
        for w in range(3):
            ...
            suma = suma + 1
        count = count + 1
data_corre.to_csv(OUTPUT+"tabla_correlacion_MAIAC_temp.csv", index=False)
